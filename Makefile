...
run:
	docker run --privileged -ti --rm --mac-address=$(MAC_ADDR) -e DISPLAY=$(DISPLAY) -e XDG_RUNTIME_DIR=/run/user/1000 -v /tmp/.X11-unix:/tmp/.X11-unix -v $(HOST_DIR):/home/user/$(HOST_DIR) qtocker:v$(QTOCKER_VERSION)
